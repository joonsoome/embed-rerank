...
class TestCrossAPICompatibility:
    ...
    def test_embedding_consistency_across_apis(self, client):
        ...
        # Test TEI API
        tei_response = client.post(
            "/embed",
            json={"inputs": test_text, "normalize": True}
        )
...
class TestRealBackendIntegration:
    ...
    async def test_backend_initialization(self):
        ...
        try:
            ...
            rerank_result = await backend.rerank_documents(
                "test query",
    ...
    def test_concurrent_request_handling(self, client):
        ...
        def make_request(request_id):
            try:
                response = client.post(
                    "/api/v1/embed/",
                    json={"texts": [f"Concurrent request {request_id}"]}
                )
                results.put((request_id, response.status_code, response.elapsed.total_seconds() if hasattr(response, 'elapsed') else 0))
            except Exception as e:
