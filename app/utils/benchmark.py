...
class BackendBenchmark:
    ...
    async def compare_backends(
        ...
        for backend_type in backend_types:
            try:
                result = await self.benchmark_backend(
                    backend_type, test_texts, iterations
                )
                comparison_results["backends"][backend_type] = result
            except Exception as e:
                logger.error(f"Failed to benchmark {backend_type}", error=str(e))
                comparison_results["backends"][backend_type] = {
                    "error": str(e),
                    "available": False
                }
        ...
        available_backends = {
            k: v for k, v in comparison_results["backends"].items() 
            if "error" not in v
        }
    ...
    def _generate_comparison(self, backends: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        comparison["load_time"] = {
            "fastest": fastest_load,
            "times": load_times
        }
        ...
        comparison["inference_speed"] = {
            "fastest": fastest_inference,
            "speeds": inference_speeds
        }
        
        # Optimal batch sizes
        comparison["optimal_batch_sizes"] = {
            name: result["optimal_batch_size"] for name, result in backends.items()
        }
        
        # Generate recommendations
        if fastest_load == fastest_inference:
            comparison["recommendations"].append(
                f"{fastest_load} is optimal for both load time and inference speed"
            )
        else:
            comparison["recommendations"].append(
                f"{fastest_load} loads fastest, {fastest_inference} runs fastest"
            )
