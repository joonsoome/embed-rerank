...
class OpenAIEmbeddingRequest(BaseModel):
    ...
    device_preference: Optional[str] = Field(
        default=None,
        description="⚡ Device preference: 'mps', 'cpu', or 'auto'",
        json_schema_extra={"example": "mps"}
    )
...
class OpenAIEmbeddingUsage(BaseModel):
    ...
    mlx_processing_time: Optional[float] = Field(
        default=None, 
        description="⚡ MLX backend processing time in seconds"
    )
    total_processing_time: Optional[float] = Field(
        default=None, 
        description="🕐 Total request processing time in seconds"
    )
    backend_used: Optional[str] = Field(
        default=None, 
        description="🧠 Backend that processed the request"
    )
    device_used: Optional[str] = Field(
        default=None, 
        description="💻 Device that processed the request"
    )
    batch_size_used: Optional[int] = Field(
        default=None, 
        description="📦 Actual batch size used for processing"
    )
...
async def create_embeddings(
    ...
    try:
        ...
        embedding_data = [
            ...
            for i, vector in enumerate(mlx_result.vectors)
        ]
        ...
        response = OpenAIEmbeddingResponse(
            data=embedding_data,
            model=request.model,
            usage=OpenAIEmbeddingUsage(**usage_data)
        )
...
async def list_models(
    ...
    try:
        ...
        logger.error(
            "💥 Failed to list OpenAI-compatible models",
            error=str(e),
            error_type=type(e).__name__
        )
...
async def openai_health(
    ...
    try:
        ...
        logger.error(
            "💥 OpenAI compatibility health check failed",
            error=str(e),
            error_type=type(e).__name__
        )
